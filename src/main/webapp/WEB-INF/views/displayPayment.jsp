...
<head>
    ...
    <style>
        body {
        background-image:"https://img.freepik.com/free-vector/colorful-confetti-background-with-text-space_1017-32374.jpg";
...
<body>
    ...
    <div class="container">
        <div class="notification-icon">
            ...
            <span class="badge badge-danger" id="notification-count">1+</span>
        </div>
    </div>
...
<!-- Donor Data -->
<c:set var="donorData" value="${donorList}" />
    ...
    $(document).ready(function () {
        ...
        function calculateTotalSum() {
            ...
            $("table tbody tr:visible").each(function () {
                var amount = parseFloat($(this).find("td:eq(6)").text());
                if (!isNaN(amount)) {
                    totalAmount += amount;
                }
            });
...
<script>
    ...
    var countryData = [];
    var countryDonors = {};
    <c:forEach var="user" items="${donorData}">
        var country = "${user.address}";
        if (countryDonors[address]) {
            countryDonors[address]++;
        } else {
            countryDonors[address] = 1;
        }
    </c:forEach>
    ...
    var donationTypeLabels = ["Kpay", "WeekWavePay", "CBPay"];
    var donationTypeData = [0, 0, 0];
    <c:forEach var="donor" items="${donorData}">
        var donationType = "${paymentmethod.paymentType}";
        if (donationType === "Kpay") {
            donationTypeData[0]++;
        } else if (donationType === "WeekWavePay") {
            donationTypeData[1]++;
        } else if (donationType === "CBPay") {
            donationTypeData[2]++;
        }
    </c:forEach>
    ...
    var donationAmounts = [];
    <c:forEach var="donor" items="${donorData}">
        donationDateLabels.push("${payment.donationDate}");
        donationAmounts.push(${payment.amount});
    </c:forEach>
...
<script>
    ...
    var donorDataArray = [];
    <c:forEach var="donor" items="${donorData}">
        donorDataArray.push({ name: "${donor.firstName} ${donor.lastName}", amount: ${donor.amount} });
    </c:forEach>
    ...
    <c:forEach var="donor" items="${donorData}">
        var issueChosen = "${donateplace.placename}";
        if (issueDonationData[issueChosen]) {
            issueDonationData[issueChosen] += ${payment.amount};
        } else {
            issueDonationData[issueChosen] = ${payment.amount};
        }
    </c:forEach>
...
<div class="container">
    ...
    <div class="form-group">
        <label for="searchDonor">Search by Donor Name:</label>
        <input type="text" class="form-control" id="searchDonor" placeholder="Enter donor name" oninput="filterDonors()">
    </div>

    <!-- Display total sum -->
    <div class="alert alert-success" id="totalAmountDisplay" style="display: none;">
...
<button type="button" class="btn btn-primary" id="showThisMonthsDonors">This Month's Donors</button>
    ...
    <!-- Include jQuery (you can add this to your HTML header) -->
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>

    <script>
        function checkForNewDonors() {
            $.ajax({
                url: '/donarList', // URL to check for new donors (adjust accordingly)
                method: 'GET',
...
<button type="button" class="btn btn-primary" id="showThisWeeksDonors">This Week's Donors</button>
<script>
    $(document).ready(function () {
...
<button type="button" class="btn btn-secondary" id="showAllDonors">Show All Donors</button>
...
<div class="container">
    ...
    <!-- Filter by Country -->
    <div class="form-group">
        <label for="filterCountry">Filter by Address:</label>
        <select class="form-control form-control-sm" id="filterCountry">
    ...
    <div class="form-group">
        <label for="filterIssue">Filter by Issue Chosen:</label>
        <select class="form-control form-control-sm" id="filterIssue">
            <option value="">All</option>
    ...
    <div class="form-group">
        ...
        <select class="form-control form-control-sm" id="filterDonationType">
            <option value="">All</option>
